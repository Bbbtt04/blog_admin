      ...
      <el-card class="box-card">
        <template #header>
          <div class="card-header">
            <span>分类列表</span>
            <el-button class="button" text @click="getTCategoryList">获取最新列表</el-button>
          </div>
        </template>
        <el-tag
            v-for="item in items"
...
import {createCategoy, getCategory, removeCategory} from "@/api/categories";
...
const submitForm = (formEl: FormInstance | undefined) => {
  if (!FormData.name) ElMessage.error('请输入标签')
    ...
    createCategoy(FormData).then(data => {
